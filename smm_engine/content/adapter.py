...
Каждый пост должен иметь СТРОГУЮ премиальную структуру:
1. Заголовок: Начинается с тематического эмодзи и написан СТРОГО В UPPERCASE (ЗАГЛАВНЫМИ БУКВАМИ). Например: "⚡ GEMMA 2: НОВАЯ ЭРА ИИ ОТ GOOGLE".
2. Лид-абзац (Hook): Одно короткое интригующее предложение-крючок, объясняющее суть новости.
3. Основной разбор (Commentary): Короткий абзац (1-2 предложения), объясняющий важность события. Ключевые термины или фичи выдели жирным текстом HTML <b>Важные слова</b>.
4. Детали (Сворачиваемый блок): Списки и подробности ОБЯЗАТЕЛЬНО оберни в раскрывающуюся цитату: <blockquote expandable>Содержимое списка</blockquote>. Каждый пункт должен начинаться со СТРОГОГО маркера "{list_bullet}".
5. Интерактив (Вопрос в конце): Закончи пост открытым вовлекающим вопросом для гиков и разработчиков, стимулирующим обсуждение в комментариях.

Правила стиля:
{style_rules_str}
- Длина всего текста body должна быть СТРОГО до {max_length} символов! Пиши только самое важное, без лишней "воды".
...
2. Сформулируй цепляющий заголовок в UPPERCASE (поле "title", БЕЗ HTML-тегов вроде <b>, так как он стилизуется автоматически).
3. Напиши тело поста (поле "body"), придерживаясь структуры выше (Лид -> Разбор с <b> -> Детали в <blockquote expandable> -> Вопрос).
4. НЕ добавляй хэштеги и ссылку на источник в поле "body" — они будут добавлены автоматически.
5. Верни СТРОГО JSON-объект в следующем формате:
{{
  "title": "ЗАГОЛОВОК С ЭМОДЗИ В ВЕРХНЕМ РЕГИСТРЕ",
  "body": "Тело поста..."
}}
"""
        try:
